...
Where _specifically noted_, a number denoting a duration can be suffixed
by one of the case-insensitive suffixes `h`, `m`, `s` and `ms`, for
denoting durations in hours, minutes, seconds and milliseconds, respectively.
...
Note that if an explicit unit is not specified, then it is configuration
parameter specific whether the duration is interpreted as seconds or
milliseconds.

...
void ClustrixMonitor::choose_hub()
{
    ...
    if (pHub_con)
    {
        if (!m_pHub_server) // We weren't monitoring anything yet.
        {
            MXS_NOTICE("Monitoring Clustrix cluster state using node %s:%d.",
                       pHub_server->address, pHub_server->port);
        }
        else if (pHub_server != m_pHub_server)
        {
            MXS_NOTICE("Monitoring Clustrix cluster state using %s:%d (used to be %s:%d).",
                       pHub_server->address, pHub_server->port,
                       m_pHub_server->address, m_pHub_server->port);
        }
